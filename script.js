...
/**
 * @overview The "PPC from SEO Excluder" script applies negative keywords
 *     from a spreadsheet, which imports queries from Google Search Console using add-on
 *     into multiple campaigns in your Google Ads account using a shared keyword list.
 *
 * @author The "PPC from SEO excluder" script is based on "Master Negative List Script"
 * 		developed by Google Ads Scripts Team [adwords-scripts@googlegroups.com].
 * 		The original script is modified by Stanislav Berkutov [stas.berkutov@gmail.com]
 *
 * @version 1.0
 *
 */
...
function sendEmail(config, emailParams) {
  ...
  if (emailParams.Status == 'OK') {
    summary = 'The PPC from SEO Excluder script successfully processed ' +
        'Customer ID: ' + AdsApp.currentAccount().getCustomerId() +
        ' and synced a total of ' + emailParams.KeywordCount + ' keywords.';
  } else {
    summary = 'The PPC from SEO Excluder script failed to process ' +
        'Customer ID: ' + AdsApp.currentAccount().getCustomerId() +
        ' and synced a total of ' + emailParams.KeywordCount + ' keywords.';
  }
  html.push('<html>',
